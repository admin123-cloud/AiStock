...
OUT_DIR = ROOT / "reports" / "gen3_market_router_fourstate_g2v4_g3range_v1"
...
FOUR_STATE_CONTEXT = ROOT / "reports" / "g2_open_v1_market_gate_per_market" / "g3_eval_per_market" / "labeled_candidates.csv"
...
def load_four_state_context() -> pd.DataFrame:
    ...
    ctx = ctx[[c for c in keep if c in ctx.columns]].dropna(subset=["entry_date_ts"])
    return ctx.drop_duplicates("entry_date_ts", keep="first")


def attach_four_state(candidates: pd.DataFrame) -> pd.DataFrame:
    d = candidates.copy()
    ctx = load_four_state_context()
    d = d.merge(ctx, on="entry_date_ts", how="left")
    d["market_style"] = d["market_style"].fillna("unknown_context")
    d["four_state_coverage"] = d["market_style"].ne("unknown_context")
    return d


def route_allowed(row: pd.Series, variant: str) -> bool:
    style = str(row.get("market_style", "unknown_context"))
    route = str(row.get("route", ""))
    if variant == "fourstate_strict":
        if style == "standard_uptrend":
            return route == "g2_v4_strong"
        if style == "standard_range":
            return route == "g3_range_balanced_77"
        return False
    if variant == "fourstate_balanced":
        if style in {"standard_uptrend", "weak_rebound"}:
            return route == "g2_v4_strong"
        if style == "standard_range":
            return route == "g3_range_balanced_77"
        return False
    if variant == "fourstate_range_plus_unknown_pre2024":
        if style in {"standard_uptrend", "weak_rebound"}:
            return route == "g2_v4_strong"
        if style in {"standard_range", "unknown_context"}:
            return route == "g3_range_balanced_77"
        return False
...
def route_candidates(g2: pd.DataFrame, g3: pd.DataFrame, variant: str) -> pd.DataFrame:
    ...
    if variant.startswith("fourstate_"):
        d = pd.concat([g2, g3], ignore_index=True, sort=False)
        d = attach_four_state(d)
        d = d[d.apply(lambda r: route_allowed(r, variant), axis=1)].copy()
        d["route_priority"] = d["route"].map({"g2_v4_strong": 0, "g3_range_balanced_77": 1}).fillna(9)
        d = d.sort_values(["entry_date_ts", "route_priority", "rank_key"], ascending=[True, True, False])
        return d.groupby("entry_date_ts", as_index=False).head(1).copy()
    raise ValueError(variant)
...
def write_report(summary: pd.DataFrame, windows: pd.DataFrame, routes: pd.DataFrame, conc: pd.DataFrame, top: pd.DataFrame, worst: pd.DataFrame) -> None:
    ...
    lines = [
        ...
        "- 四态上下文来源：`reports/g2_open_v1_market_gate_per_market/g3_eval_per_market/labeled_candidates.csv`，可覆盖 2024-01-30 至 2026-05-29 的入场日；早于该范围的样本标记为 `unknown_context`。",
        ...
        "- `fourstate_strict`：严格四态路由。`standard_uptrend` 标准主升只允许 G2 强势；`standard_range` 标准震荡只允许 G3 横盘；`weak_rebound` 弱势反弹和 `unknown_context` 暂不交易。",
        "- `fourstate_balanced`：均衡四态路由。标准主升和弱势反弹允许 G2 强势；标准震荡允许 G3 横盘；未知上下文不交易。",
        "- `fourstate_range_plus_unknown_pre2024`：承认早期上下文缺口的补位测试。主升/弱反弹用 G2 强势；震荡和未知上下文用 G3 横盘。",
        "- `base_net`：使用原交易腿净收益。",
        ...
        "",
        "## 判断",
        "- 这一步用于验证“G3 应吸收 G2 v4 强势打法，并按市场环境路由”，不是最终实盘候选。",
        "- 若四态路由不显著伤害 G2 强势收益，同时保留 G3 横盘在非强势期的补位价值，下一步才值得做真实 G2 引擎逐bar重放。",
        "- 若严格四态明显砍掉收益，说明弱势反弹里的 G2 强势打法不能简单禁用，应作为独立弱反弹/强势探针链路继续研究。",
    ]
...
def main() -> None:
    ...
    variants = [
        "g2_v4_legcash_only",
        "g3_range_only",
        "router_g2_priority_plus_g3",
        "fourstate_strict",
        "fourstate_balanced",
        "fourstate_range_plus_unknown_pre2024",
    ]
    for variant in variants:
        ...
        for profile in PROFILES:
            ...
            if variant == "fourstate_balanced" and profile["profile"] == "base_net":
