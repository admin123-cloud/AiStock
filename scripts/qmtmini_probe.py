...
def run_probe(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        ...
        capabilities = market.probe_capabilities(args.stock_code)
        summary["market"]["capabilities"] = capabilities
        if not capabilities.get("ok"):
            summary["ok"] = False
        if capabilities.get("capabilities", {}).get("full_tick", {}).get("ok"):
            ...
            summary["market"]["full_tick"] = capabilities.get("capabilities", {}).get("full_tick", {})
    ...
    if args.skip_dry_run_order:
        summary["dry_run_order"] = {"ok": True, "skipped": True}
    else:
...
def main() -> int:
    ...
    parser.add_argument("--skip-dry-run-order", action="store_true")
