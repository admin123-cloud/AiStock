...
if ($Plan) {
  [pscustomobject]@{ Execute=$PythonExe; Arguments=$TaskArguments; WorkingDirectory=$RootDir; NotificationsEnabled=$NotificationsEnabled; RepairsEnabled=[bool]$EnableRepairs } | ConvertTo-Json
  exit 0
}
