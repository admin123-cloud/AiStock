...
def bar_times(period: int) -> list[str]:
...
def build_delivery_calendar(client, *, days: int = 30, now: datetime | None = None) -> dict[str, Any]:
    ...
    exclusions = client.query(
        ...
        f"WHERE evidence_url != '' AND start_date<=toDate('{end}') AND end_date>=toDate('{start}') SETTINGS max_execution_time=10"
    ).result_rows
    expected = {}
    exceptions = {}
    for day in dates:
        ...
        for period in (0, 5, 15, 30, 60):
            ...
            try:
                ...
                for day in dates:
                    ...
                    times = set(expected_times(day, period, now)) if period else set()
                    due = bool(times) if period else day < end or now.time() >= time(16, 10)
    ...
    try:
        ...
        sector_cells = [{'date': day, 'status': 'not_due' if day == end and now.time() < time(16, 10) else 'unverified' if by_date.get(day) else 'missing', 'actual': by_date.get(day, 0), 'expected': None, 'coverage': None} for day in dates]
    except Exception as exc:
