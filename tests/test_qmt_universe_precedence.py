...
def test_real_sql_or_does_not_bypass_quit_and_realtime_dates(monkeypatch):
    ...
    db.execute('CREATE TABLE stocks (code TEXT,name TEXT,type TEXT,quit INTEGER,list_date TEXT,listing_status TEXT,delist_date TEXT)')
    day=datetime.now(collector.SH_TZ).date().isoformat()
    db.executemany('INSERT INTO stocks VALUES(?,?,?,?,?,?,?)',[
        ('600001.SH','active','stock',0,'2000-01-01','active',None),
        ('600002.SH','retired','stock',1,'2000-01-01','active','2020-01-01'),
        ('600003.SH','future','stock',0,'2099-01-01','pending_listing',None),
        ('600004.SH','expires_today','stock',0,'2000-01-01','active',day),
        ('000001.SH','active index','index',0,None,'active',None),
        ('000002.SH','retired index','index',1,None,'active','2020-01-01'),
        ('301686.SZ','pending','stock',0,None,'pending_listing',None),
    ])
